...
const showEnLarge = imageUrl => {
    ...
    document.getElementById("enLargePhoto").style.backgroundImage = `url(${imageUrl})`;
                ...
                for(let i = 0, l = data[_date].length; i < l; i++) {
                    if(data[_date][i]?.type === 'youtube') {
                        ...
                        if (data[_date][i]?.share && data[_date][i]?.sellDate && data[_date][i]?.sellPrice && data[_date][i]?.buyDate && data[_date][i]?.buyPrice && data[_date][i]?.price) {
                            ...
                            events += `<br/><span class="trade">${getEmoji(0)} ${data[_date][i]?.share}<br/>${[data[_date][i]?.buyDate.substring(2, 4), '.', data[_date][i]?.buyDate.substring(4, 6), '.', data[_date][i]?.buyDate.substring(6, 8)].join('')} ${(data[_date][i]?.buyPrice.replace(/\,/g, '') - 0).toLocaleString("ko-KR") + '원 청약 납입'}</span>`;
                        }
                        events += `</p>
                        ${
